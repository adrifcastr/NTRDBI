...
static list_item download = {"Download", COLOR_TEXT, action_install_titledb};
...
static void titledb_action_open(linked_list* items, list_item* selected) {
    ...
    list_display("NTRDB Action", "A: Select, B: Return", data, titledb_action_update, titledb_action_draw_top);
}
...
void titledb_open() {
    ...
    list_display("NTRDB", "A: Select, B: Return, X: Refresh", data, titledb_update, titledb_draw_top);
}